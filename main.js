...
const bufferTexture = () => {
  ...
  bufferMaterial = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uMouse: {
        value: { x: window.innerWidth * 0.5, y: window.innerHeight * 0.5 },
      },
      uTime: { value: 0 },
      uCol: { value: 0 },
      uSpeed: { value: 0 },
...
const handleResize = () => {
  bufferMaterial.uniforms.uRes.value = new THREE.Vector2(
    window.innerWidth,
    window.innerHeight
  );
  renderer.setSize(window.innerWidth, window.innerHeight);
...
*/
  ...
  speed = THREE.MathUtils.lerp(
    speed,
    Math.max(Math.abs(prevX - mouseX), Math.abs(prevY - mouseY)),
    0.05
  );
  bufferMaterial.uniforms.uSpeed.value = speed * 0.1;
  prevX = mouseX;
  prevY = mouseY;
